...
class Serializer {
    ...
    parse_xlsx(xlsx_object, strategy_) {
    ...
    read_text_row(row, strategy_) {
        let ret_lead = new Lead();
        const set_parameter = (lead_, chparameter_, value) => {
            switch (chparameter_) {
        ...
        if (mres.length < normalized_strategy_.scheme.length + 1)
            ...
            for (let i = 0; i < normalized_strategy_.scheme.length; i++) {
                set_parameter(ret_lead, normalized_strategy_.scheme[i], mres[i + 1]);
            }
        }
        return ret_lead;
    }
    parse_text(text, strategy_) {
        let rows = text.split('\n');

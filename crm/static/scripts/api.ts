...
export class api{
    ...
    constructor() {
        this.xhrs = {file: this.init_xhr(API_C.XHR_TIMEOUT5K),
                transfer: this.init_xhr(API_C.XHR_TIMEOUT5K),
                transferstatus: this.init_xhr(API_C.XHR_TIMEOUT5K),
                message: this.init_xhr(API_C.XHR_TIMEOUT5K),
            };
        //this.init_file_xhr();
    ...
    public send_new_leads(leads, url:string, f_obj: FileModalWindow) {  //ADDED  ": FileModalWindow"
    ...
    get_status(callback: CallableFunction, ts_obj) {
        let url = this.makeurl(API_C.URL_TRANSFERSTATUS);
    ...
    public get_message(message_object: MessageReceiver) {
        let url = this.makeurl(API_C.URL_RECEIVE_MESSAGE);
    ...
    private get_cc_prefix(): string {
        return '/'+location.pathname.split('/')[1];
    }

    public makeurl(url:string) {
        return url;
        return this.get_cc_prefix() + url;
    }
